...
8. 학생이 작성한 자기평가 내용(${selfDescription})을 자연스럽게 분석하고 수렴하여, 본인의 성찰 성향을 좋게 반영해 줍니다.`;
      ...
      prompt = `다음 학생의 강점과 약점, 자기평가를 확인한 후, 학생의 자존감을 높이고 긍정적인 성장을 진심으로 독려하는 '성장 맞춤형 상담 편지(조언)'를 300자 정도 내외로 적어줘.
...
[중요 작성 규칙]
1. 격려가 가득 담긴 다정하고 친근하며 정겨운 대화체('~길 바랄게', '~고마워', '~응원할게', '~하는 모습이 정말 멋져')로 한 명의 따뜻한 조력자로서 말해주세요.
2. 이름(${name})을 중간에 자주 자연스럽게 불러주어 다정함을 주되, 경직된 주격 표기('학생은~')는 일체 배제합니다.
3. 강점(장점)을 크게 칭찬하며 학생이 자신감을 갖도록 깊이 공감해 주고, 약점은 단점이 아닌 '앞으로 날개를 펼쳐 발전시킬 멋진 성장 영역'으로 부러지지 않게 구체적인 실천 로드맵을 곁들여 안심시켜 줍니다.
4. 격정적인 느낌표(!!!)나 이모티콘 등의 남발보다는, 세련되면서도 진심 어린 차분하고 진솔한 말투로 작성합니다.
5. 서술 시 불필요한 마크업 기호(*, ID, Headers, \` 등)는 쓰지 않고, 가독성 좋고 따뜻한 편지 단락 구조로 써야 합니다.
6. 300자 내외로 구성해 주세요.`;
    }
