...
8. 학생이 작성한 자기평가 내용(${selfDescription || "없음"})을 자연스럽게 분석하고 수렴하여, 본인의 성찰 성향을 좋게 반영해 줍니다.`;
      ...
      systemInstruction = "너는 교육심리학적 전문성과 다정한 위로 능력을 갖춘 대한민국의 따뜻하고 친근한 멘토 선생님이야.";
      prompt = `다음 학생의 강점, 보완하고 싶은 점, 그리고 스스로 작성한 한 줄 성찰을 바탕으로 학생의 자존감을 높이고 성장을 따뜻하게 응원하는 '성장 맞춤형 조언 편지'를 정성껏 작성해줘.

[학생 정보]
이름: ${name}
선택한 나의 장점(강점): ${strengthsStr || "없음"}
이번에 더 노력하고 싶은 점(약점): ${weaknessesStr || "없음"}
내가 돌아본 나의 이번 학기 일지(자기평가): ${selfDescription || "없음"}

[중요 작성 규칙]
1. 위압적이거나 공식적인 공문서 서식을 절대 사용하지 말고, 다정하고 친근한 어조(반말, 예: '~하길 바랄게', '~하는 모습이 정말 멋져', '~선생님은 너의 예쁜 성장을 언제나 응원해!')로 말해줘.
2. 학생이 선택한 장점(${strengthsStr})과 단점/더 보완할 점(${weaknessesStr})의 구체적인 명칭을 자연스럽게 언급하며, '너는 정말 멋진 OO라는 강점을 가지고 있구나!', '다만 스스로 OO가 아주 조금 아쉬웠다고 적어준 깊은 생각을 보고 감동했어' 등의 흐름으로 구체적으로 반영해 주어야 해.
3. 절대로 '생활기록부', '생기부', '나이스', '대시보드 기획실', '공식 평가', '기재 가이드라인' 등과 같은 행정 및 학교 공무 단어를 사용해서는 안 돼. 학생은 이것을 부담스러운 생기부 기입 절차가 아니라 가벼운 마음으로 스스로를 채워가며 따뜻한 다독임을 얻는 '자가 진단/자기평가' 성장방으로 믿고 있어.
4. 이름(${name})을 중간중간 다정하게 부르며 오롯이 그 학생만을 위한 일기장 속 조언 편지 느낌으로 작성해 줘.
5. 중간에 마크다운 기호(*, #, - 등)는 절대 사용하지 말고, 가독성 좋고 편안한 줄바꿈 단락들로만 보기 좋게 300자 내외로 구성해 줘.`;
    }
